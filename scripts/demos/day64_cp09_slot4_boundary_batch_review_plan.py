"""Day 64 step 1: audit the completed Day 63 boundary batch at CP09."""
...
def frozen_path(config, path_key, hash_key):
    ...
    if not path.is_file() or sha256_file(path) != config["source"][hash_key]:
        raise ValueError(f"Frozen Day 64 evidence changed: {path_key}")
    return path
...
def load_frozen_json(config, path_key, hash_key, expected_task_key, expected_status="success"):
    ...
    if report.get("task") != config["source"][expected_task_key] or report.get("status") != expected_status:
        raise ValueError(f"Frozen Day 64 metadata is invalid: {path_key}")
    return path, report
...
def validate_execution_lock(config):
    ...
    if any(execution.get(key) is not True for key in allowed):
        raise ValueError("Day 64 review work is not enabled.")
    if any(value is not False for key, value in execution.items() if key not in allowed):
        raise ValueError("Day 64 enabled execution or source modification.")


def validate_approval_and_marker(config, approval, marker, result, result_path):
    checks = (
        approval.get("decision_id") == "AP-DAY62-001",
        ...
        marker.get("approval_sha256") == config["source"]["day62_approval_sha256"],
        marker.get("decision_id") == approval.get("decision_id"),
        marker.get("maximum_batch_execution_count") == 1,
        marker.get("maximum_case_execution_count") == 9,
        marker.get("rerun_released") is False,
        Path(marker.get("run_directory", "")).resolve() == result_path.parent.resolve(),
    ...
    if not all(checks):
        raise ValueError("The Day 63 authorization-consumption evidence is inconsistent.")
...
def validate_one_case(config, case_path, expected_id, expected_offset, batch_row, reproduction):
    if not case_path.is_file():
        raise FileNotFoundError(f"Day 63 case report is missing: {case_path}")
    report = json.loads(case_path.read_text(encoding="utf-8"))
    tolerance = float(config["review_criteria"]["numeric_tolerance"])
    checks = (
        report.get("task") == "day63_approved_day25_boundary_case",
        ...
        report.get("continuous_tolerance_claimed") is False,
        report.get("balanced_acceptance_pass") is bool(batch_row["balanced_acceptance_pass"]),
        ";".join(report.get("failed_metrics", [])) == str(batch_row["failed_metrics"]),
    )
    if not all(checks):
        raise ValueError(f"Day 63 case safety or acceptance evidence failed: {expected_id}")
    ...
    if any(not math.isclose(float(left), float(right), abs_tol=tolerance) for left, right in comparisons):
        raise ValueError(f"Day 63 case metrics disagree with the batch summary: {expected_id}")
    ...
    reproduction_ok = (
        float(reproduction["maximum_spot_summary_difference_um"]) <= spot_limit,
        float(reproduction["maximum_mtf_summary_difference"]) <= mtf_limit,
        reproduction.get("acceptance_pass_matches") is True,
        reproduction.get("failed_metrics_match") is True,
    )
    if not all(reproduction_ok):
        raise ValueError(f"Day 63 historical reproduction failed: {expected_id}")
    spot_path = Path(report["spot_text"]).resolve()
    mtf_path = Path(report["mtf_text"]).resolve()
    working_path = Path(report["working_copy"]).resolve()
    if not all(path.is_file() for path in (spot_path, mtf_path, working_path)):
        raise FileNotFoundError(f"A raw or working-copy file is missing: {expected_id}")
    if sha256_file(working_path) != config["source"]["focused_model_sha256"]:
        raise ValueError(f"The disk working copy changed: {expected_id}")
    return {
        "case_id": expected_id,
        "offset_mm": float(expected_offset),
        "acceptance_pass": bool(batch_row["balanced_acceptance_pass"]),
        "failed_metrics": str(batch_row["failed_metrics"]),
        ...
        "working_copy_sha256": sha256_file(working_path),
...
def validate_batch(config, result):
    ...
    checks = (
        ...
        result.get("acceptance_pass_count") == int(criteria["expected_acceptance_pass_count"]),
    ...
    if not all(checks) or len(result.get("case_reports", [])) != len(ids):
        raise ValueError("The Day 63 batch-level evidence is incomplete or unsafe.")
    audits = []
    for index, (case_id, offset) in enumerate(zip(ids, offsets)):
        ...
        if not math.isclose(float(row["offset_mm"]), offset, abs_tol=float(criteria["numeric_tolerance"])):
            raise ValueError(f"Day 63 offset mismatch: {case_id}")
        audits.append(
            validate_one_case(
                config,
                Path(result["case_reports"][index]).resolve(),
                case_id,
                offset,
                row,
                result["historical_reproduction"][index],
            )
        )
    return audits
...
def validate_decision(config):
    ...
    if config["decision"]["decision_status"] != expected:
        raise ValueError("The Day 64 decision status is incorrect.")
    released = {"slot_04_boundary_batch_review_completed", "slot_05_release_request_eligible"}
    permissions = config["permissions"]
    if any(permissions.get(key) is not True for key in released):
        raise ValueError("Day 64 review permissions are incomplete.")
    if any(value is not False for key, value in permissions.items() if key not in released):
        raise ValueError("Day 64 released execution or an engineering claim.")
...
def prepare_review(config):
    ...
    result_path, result = load_frozen_json(
        config, "day63_batch_result", "day63_batch_sha256", "expected_day63_task"
    )
    marker_path, marker = load_frozen_json(
        config,
        "authorization_marker",
        "authorization_marker_sha256",
        "expected_marker_task",
        "consumed_before_zosapi_execution",
    )
    ...
    historical_path = frozen_path(config, "historical_boundary_batch", "historical_boundary_batch_sha256")
    csv_path = frozen_path(config, "comparison_csv", "comparison_csv_sha256")
    validate_approval_and_marker(config, approval, marker, result, result_path)
...
def main():
    ...
    print("========== DAY 64 CP09 SLOT-4 BOUNDARY-BATCH REVIEW PLAN ==========")
    print("Mode: PLAN ONLY")
    print("No review record, ZOS-API connection, optical calculation or Slot 5 release will occur.")
    print(f"Decision: {config['decision']['decision_id']} -> {config['decision']['decision_status']}")
    print(f"Day63 batch result: {review['result_path']}")
    print(f"Day63 batch SHA256: {config['source']['day63_batch_sha256']}")
    ...
    for audit in review["case_audits"]:
        state = "PASS" if audit["acceptance_pass"] else f"FAIL ({audit['failed_metrics']})"
        print(f"  {audit['case_id']} ({audit['offset_mm']:+.3f} mm): {state}; Spot diff={audit['historical_spot_difference_um']:.9f} um, MTF diff={audit['historical_mtf_difference']:.9f}")
    print("Slot 5 execution approved: False")
    print()
    print("[PASS] Day62 approval and pre-execution consumption marker verified")
    print("[PASS] Nine case identities, offsets, summaries and acceptance signatures verified")
    print("[PASS] Nine case JSON files and eighteen raw analysis files fingerprinted")
    print("[PASS] Historical optical and acceptance evidence reproduced at every case")
    print("[PASS] All connections closed and all model/copy hashes unchanged")
    print("[PASS] Review PASS remains separate from Slot 5 execution approval")
