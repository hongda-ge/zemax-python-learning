...
if __name__ == '__main__':
    ...
    # D12：创建 MTF/Spot 分析结果输出文件夹
    out_dir = Path(r"C:\Users\20181\Desktop\Zemax\02_zosapi_python\results\D12_analysis_export")
    ...
    csv_path = r"C:\Users\20181\Desktop\Zemax\02_zosapi_python\results\D12_cooke_lde_after_modify.csv"
    ...
    save_path = r"C:\Users\20181\Desktop\Zemax\02_zosapi_python\results\D12_cooke_surface3_thickness_plus1.zmx"
    TheSystem.SaveAs(save_path)

    print("\nModified Zemax file saved to:", save_path)

    # ===== D12：运行 FFT MTF 分析并导出 txt =====
    print("\n===== D12 Export FFT MTF =====")
    mtf = TheSystem.Analyses.New_Analysis(constants.AnalysisIDM_FftMtf)
    mtf.ApplyAndWaitForCompletion()
    mtf_results = mtf.GetResults()
    mtf_txt_path = out_dir / "fft_mtf.txt"
    mtf_results.GetTextFile(str(mtf_txt_path))
    mtf.Close()

    print("FFT MTF 已导出：", mtf_txt_path)

    # ===== D12：运行 Standard Spot Diagram 并导出 txt =====
    print("\n===== D12 Export Standard Spot Diagram =====")
    ...
    spot_txt_path = out_dir / "standard_spot.txt"
    spot_results.GetTextFile(str(spot_txt_path))

    spot.Close()

    print("Standard Spot Diagram 已导出：", spot_txt_path)
